...
function card(product: Product, recommendation: Recommendation, index: number): string {
  ...
  return `<article class="recommendation-card"><div class="recommendation-rank">0${index + 1}</div><div class="recommendation-card-body"><p class="eyebrow">پیشنهاد ${index + 1} · ${recommendation.score.overall}%</p><h2>${escapeHtml(product.name)}</h2><p class="recommendation-reason">${escapeHtml(recommendation.reason.explanation ?? "هماهنگ با انتخاب‌های شما")}</p><dl class="recommendation-specs"><div><dt>فرم</dt><dd>${escapeHtml(shape)}</dd></div><div><dt>جنس</dt><dd>${escapeHtml(material)}</dd></div><div><dt>ابعاد</dt><dd>${escapeHtml(size)}</dd></div></dl><div class="recommendation-card-actions"><button class="button button-secondary recommendation-save" type="button" data-save-choice="${escapeHtml(recommendation.variantId.id)}">ذخیره در انتخاب‌های من</button><button class="button button-primary" type="button" data-add-to-cart="${escapeHtml(recommendation.variantId.id)}">انتخاب برای خرید</button></div></div></article>`;
}
